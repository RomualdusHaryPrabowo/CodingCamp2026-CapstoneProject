...
function splitPositioningClasses(className) {
  if (!className) return { positioning: "", styling: "" };
  ...
  classes.forEach((c) => {
    const isPositioning = 
      /^(-?(left|right|top|bottom|translate)-\S+|.*:(-?(left|right|top|bottom|translate)-\S+))$/.test(c);
...
function CarouselPrevious({
  ...
  const { positioning, styling } = splitPositioningClasses(className);
  ...
  return (
    ...
    >
      <Button
        ...
        size={size}
        className={cn(
          "size-8 rounded !p-0 flex items-center justify-center",
